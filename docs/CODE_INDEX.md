...
| `event-detail-tabs.tsx`           | Accessible event-detail tablist, badges, keyboard navigation, and intent hooks |
| `event-detail-panel-skeleton.tsx` | Contextual accessible fallback for async event-detail panels                   |
| `event-detail-guests-panel.tsx`   | Deferred guest management panel and on-demand guest modals                     |
| `event-detail-guests-state.ts`    | Lightweight guest-list view state retained by the route shell                  |
| `event-detail-settings-panel.tsx` | Deferred sharing, sections, design, and access settings group                  |
| `preload-event-detail-panel.ts`   | Stable panel import map used for pointer/focus preloading                      |
...
| `/orders`              | `(app)/orders/page.tsx`              | Legacy server redirect to `/events`                            |
| `/orders/[id]`         | `(app)/orders/[id]/page.tsx`         | Legacy server redirect to `/events`                            |
| `/clients`             | `(app)/clients/page.tsx`             | Client management (root only)                                  |
| `/users`               | `(app)/users/page.tsx`               | User management (root only)                                    |
| `/users/[id]/clients`  | `(app)/users/[id]/clients/page.tsx`  | User identity, searchable memberships, and deferred management |
