      ...
      <details open={proposalOpen} onToggle={(event) => setProposalOpen(event.currentTarget.open)} className="border-b border-border-subtle px-4 py-4 sm:px-5">
        ...
        <form onSubmit={propose} className="mt-4 space-y-4">
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <PolicySelect label="Aprobación de gates" value={draft.gateApprovalMode} onChange={(value) => updateDraft('gateApprovalMode', value as PolicyProposalDraft['gateApprovalMode'])} options={[[ '', 'Heredar' ], ['human', 'Humano por tarea'], ['delegated', 'Roles independientes']]} />
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border-subtle bg-surface-soft p-3">
            <p className="max-w-3xl text-xs leading-5 text-ink-muted">Los campos vacíos heredan la capa superior, salvo las referencias en modo release: allí una lista vacía queda registrada explícitamente. La delegación exige una propuesta durable aprobada y evidencia independiente; nunca se admite en un override temporal. Sólo se guardan nombres, nunca valores. Ramas con wildcard y workflows fuera de <span className="font-mono">.github/workflows</span> se rechazan.</p>
