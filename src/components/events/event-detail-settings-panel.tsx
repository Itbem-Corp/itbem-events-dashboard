...
function SettingsDisclosure({
  ...
  return (
    <details
      ...
      className="group overflow-hidden rounded-2xl border border-white/[0.09] bg-zinc-900/48 shadow-[0_18px_48px_rgba(0,0,0,0.16)] ring-1 ring-black/15"
    >
      <summary className="flex min-h-20 cursor-pointer list-none items-center gap-4 px-4 py-4 transition-colors hover:bg-white/[0.035] focus-visible:ring-2 focus-visible:ring-indigo-300 focus-visible:outline-none focus-visible:ring-inset sm:px-5 [&::-webkit-details-marker]:hidden">
        ...
        <span className="min-w-0 flex-1">
          <span className="block text-[10px] font-semibold tracking-[0.16em] text-zinc-500 uppercase">{eyebrow}</span>
          <span className="mt-0.5 block text-sm font-semibold text-zinc-100 sm:text-base">{title}</span>
          <span className="mt-0.5 block text-xs leading-5 text-zinc-500 sm:text-sm">{description}</span>
        </span>
        <ChevronDownIcon
          aria-hidden="true"
          className="size-5 shrink-0 text-zinc-500 transition-transform duration-200 group-open:rotate-180 motion-reduce:transition-none"
        />
      </summary>
