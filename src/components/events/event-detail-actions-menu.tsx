...
export function EventDetailActionsMenu({
  ...
  return (
    <div className="absolute top-full left-0 z-30 mt-2 w-60 rounded-xl border border-white/10 bg-zinc-900/95 p-1 shadow-2xl shadow-black/40 backdrop-blur-xl sm:right-0 sm:left-auto">
      <button
        type="button"
        onClick={handleOpenPreview}
        disabled={openingPreview}
        className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-left text-sm text-zinc-300 transition-colors hover:bg-white/5 hover:text-white focus-visible:bg-white/5 focus-visible:outline-none disabled:opacity-50"
      >
        <ArrowTopRightOnSquareIcon className="size-4 text-zinc-500" />
      ...
      >
        <PencilSquareIcon className="size-4 text-zinc-500" />
        Editar información
      </button>
